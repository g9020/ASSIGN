...
UNZIP(id) ;
	...
	;	
GAWK(folder) ;
	n sourcefiles,sourcefile,fmap,name,dir,type,x,q
	;	
	s fmap(32)="/opt/all/"_folder_"/ID32_Class_Records.csv"
	s fmap(32,"H")=$p($text(TEXT+4),"; ",2,999)
	s fmap(15)="/opt/all/"_folder_"/ID15_StreetDesc_Records.csv"
	s fmap(15,"H")=$p($text(TEXT+5),"; ",2,999)
	s fmap(21)="/opt/all/"_folder_"/ID21_BLPU_Records.csv"
	s fmap(21,"H")=$piece($text(TEXT+1),"; ",2,999)
	s fmap(28)="/opt/all/"_folder_"/ID28_DPA_Records.csv"
	s fmap(28,"H")=$p($text(TEXT+3),"; ",2,999)
	s fmap(24)="/opt/all/"_folder_"/ID24_LPI_Records.csv"
	s fmap(24,"H")=$p($text(TEXT+2),"; ",2,999)
	s fmap(30)="/opt/all/"_folder_"/ID30_SUCCESSOR_Records.csv"
	s fmap(30,"H")=$p($text(TEXT+6),"; ",2,999)
	;	
	s x=$zsearch("/opt/all/"_folder_"/*.fake")
	s q="/opt/all/"_folder_"/*.csv"
	for  set x=$zsearch(q) quit:x=""  do
	. s name=$zparse(x,"NAME")
	. ;i name'["AddressBasePremium" quit
	. s dir=$zparse(x,"DIRECTORY")
	. s type=$zparse(x,"TYPE")
	. s sourcefile=dir_name_type
	. s sourcefiles(sourcefile)=""
	. quit
	;	
	set type=""
	f  s type=$o(fmap(type)) q:type=""  do
	. s file=fmap(type)
	. s dev="OUT"_type
	. o dev:(newversion:stream:nowrap:chset="M"):file
	. use dev
	. w fmap(type,"H"),$c(10)
	. ;
	. quit
	;	
	s sourcefile=""
	f  s sourcefile=$o(sourcefiles(sourcefile)) q:sourcefile=""  do
	. close sourcefile
	. o sourcefile:(readonly)
	. s c=1
	. s eof=0
	. f  do  q:eof 
	. . u sourcefile r str i $zeof s eof=1 q
	. . s id=$p(str,",",1)
	. . s file=$get(fmap(id))
	. . s dev="OUT"_id
	. . o dev:(newversion:stream:nowrap:chset="M"):file
	. . i file="" quit
	. . s ^DLS("WRITING TO")=str_"~"_file
	. . u dev
	. . w str,$c(10)
	. . S ^DLS("WRITTEN TO")=str_"~"_file
	. . s c=c+1
	. . quit
	. close sourcefile
	. quit
...
TEXT ;
	...
	; "RECORD_IDENTIFIER","CHANGE_TYPR","PRO_ORDER","UPRN","SUCCESSOR"